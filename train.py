...
MCTS_BATCH_SIZE = 8
REPLAY_BUFFER = 30000
LEARNING_RATE = 0.01#0.1 #smaller?
BATCH_SIZE = 256
TRAIN_ROUNDS = 10
MIN_REPLAY_TO_TRAIN = 10000

BEST_NET_WIN_RATIO = 0.58

EVALUATE_EVERY_STEP = 50
EVALUATION_ROUNDS = 25
...
if __name__ == "__main__":
    ...
    #parser.add_argument("--load", default=None, help="Name of model to train")
    ...
    #if args.load:
#         options = os.listdir(saves_path)
#         """
#         Keep track of current models and use the latest one
#         """
#         for loc in reversed(options):
#             if loc.find(args.load) != -1:
#                 net.load_state_dict(torch.load(saves_path+"/"+loc))
#                 print("Loaded "+args.load+" model from "+saves_path+"/"+loc)
#                 break
    net.load_state_dict(torch.load("saves/pulsar/current_000_00350.dat")) ######
    net.train()
        
    best_net = ptan.agent.TargetNet(net)
    print(net)

    optimizer = optim.Adam(net.parameters(), lr=LEARNING_RATE)#, momentum=0.9) #can used Adam instead
    ...
    with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
        try:
            ...
            file_name = os.path.join(saves_path, "current_%03d_%05d.dat" % (count, step_idx))
            torch.save(net.state_dict(), file_name)
            sys.exit("Successfully Saved Current Model and Exited")

    file_name = os.path.join(saves_path, "student_%03d_%05d.dat" % (best_idx, step_idx))
    torch.save(net.state_dict(), file_name)
    print("Training Over After "+str(step_idx)+" Steps")

...
def two_step(field, cur):
    ...
    for i in range(7):
        ...
        for j in range(7):
            tmp_s = tmp_f
            tmp_s[j].append(3-cur)
            if _won(tmp_s) == True:
                lose.append(i)
    return lose


def three_capture(field, player):
    """
    Finds two-two positions only for horizontal/diagnol
    """
    for h in range(6):
        for c in range(7):
...
def even_odd(state, player):
    """
    'Capture' a point by surrounding it with three in a row when possible.
    When to play offensively versus defensively. 
    Know when even-odd pattern leads to definite win (3-4 columns)
    """
    #first --> odd '4th spot' for win --> should also stop traps
    #second --> even '4th spot' for win
    #block opponent's '3rd spot' from creating their '4th spot' odd/even
    #Detect two in an axis for both players
    if player == 0:
        cols = three_capture(state, player)
        pass
...
def filtration(state, action, player):
...
def play_game(mcts_stores, replay_buffer, net1, net2,
              ...
              net1_plays_first=None, device="cpu", evalu=False):
    """

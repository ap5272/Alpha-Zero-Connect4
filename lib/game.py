...
def _won(field):
    #1)add in extra spaces into board to fill out board -->  
    #2)check if [c+i] and [h+i] exist
    
    #print(field)
    for h in range(6):
        for c in range(7):
            #check if the height piece exists
            if len(field[c])-1 >= h:
                try:
                    if c < 4:
                        #check for horizontal wins
                        if field[c][h] == field[c+1][h] == field[c+2][h] == field[c+3][h]:
                            return True
                except IndexError:
                    pass
                ...
                try:
                    if h < 3:
                        ...
                        if field[c][h] == field[c][h+1] == field[c][h+2] == field[c][h+3]:
                            return True
                except IndexError:
                    pass
                ...
                try:
                    if c < 4 and h < 3:
                        #checks for rising diagonal wins
                        if field[c][h] == field[c+1][h+1] == field[c+2][h+2] == field[c+3][h+3]:
                            return True
                except IndexError:
                    pass
                ...
                try:
                    if c < 4 and h > 2:
                        #checks for falling diagonal wins
                        if field[c][h] == field[c+1][h-1] == field[c+2][h-2] == field[c+3][h-3]:
                            return True
                except IndexError:
                    pass
...
def two_step(state_int, player):
    """
    Checks if placing a piece in a column gives Connect4 to the opponent. Will  
    avoid this column(s) for as long as legally possible.
    """
    avoid = []
    for mine in range(7):
        field = decode_binary(state_int)
        field[mine].append(player)
        for yours in range(7):
            next_field = field
           
            next_field[yours].append(1-player)
            if _won(next_field) == True:
                avoid.append(mine)
    return list(set(avoid))

def one_step(state_int, player):
    """
    Checks if win by Connect4 is possible.
    If True, returns column to choose for win.
    If False, checks if Opponent has possible Connect4.
        If True, returns column to stop Connect4.
        If False, returns None.
    """
    go = []
    stop = []
    poss = possible_moves(state_int)
    for i in poss:
        try:
            next_state, you_done = move(state_int, i, player)
            next_state, op_done = move(state_int, i, 1-player)
            if you_done:
                go.append(i)
            elif op_done:
                stop.append(i)
            ...
            pass
    if len(go) > 0 and len(go) < 7:
        return go
    elif len(stop) > 0 and len(stop) < 7:
        return stop
    else:
        return None

tester = [[1, 0, 0], [0, 0], [], [], [], [0, 1, 0, 1], [1, 1, 1, 0, 1, 0]]

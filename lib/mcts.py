...
class MCTS:
    ...
    def get_policy_value(self, state_int, player, tau=1):
        """
        ...
        best = game.one_step(state_int, player)
        field = game.decode_binary(state_int)
        #second = game.two_step(state_int, player)
        """
        Select perfect move in select situations
        """
        if best is not None: #fill in connect 3's where possible
            probs = [0.0] * game.GAME_COLS
            probs[best[0]] = 1.0
        elif len(field[3]) == 0: #place a coin center if possible
            probs = [0.1] * game.GAME_COLS
            probs[3] = 0.4
        elif tau == 0:
            probs = [0.0] * game.GAME_COLS
            probs[np.argmax(counts)] = 1.0
        #elif len(second) > len(game.possible_moves(state_int)):
            #set any path that is avoidable to zero
            
         #   for i in game.possible_moves(state_int):
          #      if i in second:
           #         counts[i] += 1
            #    else:
             #       counts[i] += 2
